...
账号配置先完整校验，再原位写入；写入报错会尝试恢复原内容，但进程崩溃时不具备原子替换保障。新增用户须使用自己的内部 Key，通过至少一个启用账号的认证后才完成创建；失败则回滚。账号路由仍在首次登录使用中心后分配，分配后外部 API Key 才可调用。已有路由的认证检查仅访问所分配账号。此前已丢失配置监听的账号进程，需要排空请求后重启一次。
