...
首次使用管理密钥登录 `/admin/` 后，Web 管理中心会进入首次设置引导：依次确认邮箱域、用户初始密码、首个 CPA、OAuth/运行状态和首个用户。公开地址、额度时区、默认周额度、通知、品牌和上游代理作为可跳过的推荐项展示；未完成流程会在运行总览保留恢复入口。引导状态由控制面实时计算，Secret 不进入读取接口或浏览器持久化存储。
