...
管理中心左下角采用单行展示：`管理 API 已鉴权 | 当前版本`，版本入口无边框和底色；窄屏沿用左下角入口，并为底部版本栏预留内容空间。有正式版更新时，保留当前版本号并显示黄色心跳圆点；鼠标悬停或键盘聚焦时仅展示当前版本和最新版本，点击不跳转。系统开启减少动态效果时停止心跳动画。自动检查间隔为 15 分钟；检查失败仍保留已读取的当前版本，版本无法识别时显示“版本未知”。更新候选仅接受非 Draft、非 Prerelease 且标签严格符合 `v数字.数字.数字` 的 GitHub Latest Release。该入口仅在管理中心展示，检查更新不会执行部署。
