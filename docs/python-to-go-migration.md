...
本方案不要求新旧系统同时服务、双写、在线增量迁移或通用反向迁移框架。需要的是针对已确认 Schema 的离线转换/核对工具、最终停机备份和启动清单。
...
- 下载并校验固定 Release 和镜像；目标只拉取，构建/发布在授权工作站完成。
...
- Caddy 继续承担现有域名和 TLS，使用 external 入口模式。准备 Caddy 到 Go Edge 的持久 Docker 网络声明及唯一后端名；不安装 Nginx，不改变外部代理服务。
...
6. Caddy 连接 Go 的持久入口网络，后端使用唯一 Go Edge 名称，保留域名、TLS 和原 Edge 端口。可在停机窗口重建 Caddy，但需核对容器实际加载配置，避免单文件 bind mount 仍指向旧 inode。
