...
升级备份默认保存在运维根目录的 `backups/`，与 `runtime/` 分开。归档仅包含两份 SQLite 一致性副本、配套主密钥与其他凭据、OAuth、账号配置、部署配置和当前 Gateway 槽位；不复制日志、旧备份、生成快照或临时数据库文件。SQLite 备份先固定只读事务快照，避免持续写入导致反复重拷，并检查副本完整性；WAL 模式下业务写入可以继续。当前安装器不会被缺少此备份修复的历史 Release 安装器覆盖。
