...
运行总览 → Token 使用 → **导出周报** 可下载 XLSX，包含总览、团队、账号、个人和每日趋势五张表。默认上个完整自然周，按系统时区划分；选择本周时与上周同一时段对比。周报不受页面筛选及 Top10 限制。

周报以已保存的加权 Token 为主，同时保留原始 Token。两期均按当前团队归属汇总，历史身份仍计入；人数与账号数去重。未采集到记录不代表已确认无用量，上期为零或无记录时环比显示 `—`。导出不包含 Key 等凭据。
